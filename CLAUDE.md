...
Reson8 is a self-hosted voice and text communication platform (TeamSpeak-3-style) with an Electron desktop client and a standalone Node.js server. Current version: 1.1.0, all planned phases (1–8) complete — see "Project history" below before assuming something is unbuilt.
...
**Data model:** Prisma/Postgres (`apps/server/prisma/schema.prisma`) holds `Server`, `Channel` (self-referencing tree via `parentId`), `User`, `Role`, `Message`, `DirectMessage`, `BannedUser`, `Reaction`. Channel tree is flattened→nested in `apps/server/src/services/channel-tree.service.ts`.
...
`app-planning/progress.txt` is the authoritative build log for this project — read it before assuming a feature doesn't exist or working out why something is built a certain way. As of 09/08/2026 it's a condensed summary of phases 1–8 (all complete, current version 1.1.0) followed by an ongoing chronological log for everything after. The full unabridged entry-by-entry history — every bug's root cause, every file touched — lives in `app-planning/archive/progress-phases-1-8.txt`; reach for it when the summary compresses away a "why" you need. `app-planning/archive/` also holds the original PRDs (`PRD.md`, `Reson8_Evolutions_PRD.md`, `Reson8_Improvements_PRD.md`) that drove phases 1–6, 7, and 8 respectively — all fully implemented, kept for historical rationale rather than active reference. Release notes live in `app-planning/releases/` (one file per version).
